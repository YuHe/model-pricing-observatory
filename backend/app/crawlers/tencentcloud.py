...
class TencentCloudCrawler(BaseCrawler):
    source = "tencentcloud"

    async def crawl(self, db: AsyncSession) -> int:
        result = await db.execute(select(Provider).where(Provider.name == "腾讯云"))
        provider = result.scalar_one_or_none()
        if not provider:
            provider = Provider(name="腾讯云", type="channel", country="CN", official_url="https://cloud.tencent.com")
            db.add(provider)
            await db.commit()
