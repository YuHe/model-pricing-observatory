...
class VolcEngineCrawler(BaseCrawler):
    source = "volcengine"

    async def crawl(self, db: AsyncSession) -> int:
        result = await db.execute(select(Provider).where(Provider.name == "火山方舟"))
        provider = result.scalar_one_or_none()
        if not provider:
            provider = Provider(name="火山方舟", type="channel", country="CN", official_url="https://www.volcengine.com/product/doubao")
            db.add(provider)
            await db.commit()
        ...
        html = await crawl_with_playwright(PRICING_URL, "table", lambda x: x)
        ...
        for item in models_data:
            ...
            input_cny = input_price if input_price else None
            output_cny = output_price if output_price else None
