...
class BailianCrawler(BaseCrawler):
    source = "bailian"

    async def crawl(self, db: AsyncSession) -> int:
        result = await db.execute(select(Provider).where(Provider.name == "百炼"))
        provider = result.scalar_one_or_none()
        if not provider:
            provider = Provider(name="百炼", type="channel", country="CN", official_url="https://bailian.console.aliyun.com")
            db.add(provider)
            await db.commit()
