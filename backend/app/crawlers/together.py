...
class TogetherCrawler(BaseCrawler):
    source = "together"

    async def crawl(self, db: AsyncSession) -> int:
        result = await db.execute(select(Provider).where(Provider.name == "Together"))
        provider = result.scalar_one_or_none()
        if not provider:
            provider = Provider(name="Together", type="channel", country="US", official_url="https://together.ai")
            db.add(provider)
            await db.commit()
    ...
    def _parse_response(data: dict) -> List[Dict]:
        ...
        for item in items:
            ...
            results.append({
                "name": model_id,
                "family": TogetherCrawler._extract_family(model_id),
