...
class GroqCrawler(BaseCrawler):
    ...
    async def crawl(self, db: AsyncSession) -> int:
        ...
        if not provider:
            provider = Provider(name="Groq", type="channel", country="US", official_url="https://groq.com")
            db.add(provider)
            await db.commit()
    ...
    def _parse_response(data: dict) -> List[Dict]:
        ...
        for item in data.get("data", []):
            ...
            results.append({
                "name": model_id,
                "family": GroqCrawler._extract_family(model_id),
